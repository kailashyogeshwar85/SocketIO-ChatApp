...
/**
  Logs in the user in built.io and saves the user's session based on the chosen persist session option.
  @function login
...
/**
  Logs out the user from built.io and clears the user's session
  @function logout
...
/**
  ...
  1) When a session is persisted using <b>cookie</b>, authtoken would be extracted from it and a call to built.io would be made to fetch the whole user profile.</br>
  ...
  @param {Boolean} forceFetch Set the flag to true if you want to forcefully fetch the user's profile from built.io (Optional and defaults to false) 
  @static
  @example
    var User = Built.App('api_key').User;  // Would forcefully fetch (forceFetch set as true) the user from from built.io   
    User.getCurrentUser()
    .then(function(user){
  ...
  @return {Promise<User>}
*/
module.exports.getSession = module.exports.getCurrentUser = utility.appProxyHelper('getSession');
...
/**
  ...
  @example
    ...
    query.where('username', request.params.username);
  ...
  @return {Promise<User>}
*/
module.exports.generateAuthtoken = utility.appProxyHelper('generateAuthtoken');
...
/**
  Activates an application user on built.io
  @function activate
